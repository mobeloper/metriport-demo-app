...
import { WebAppServerInfrastructureStack } from "../lib/web-app-server-stack";
...
async function deploy() {
  ...
  //---------------------------------------------------------------------------------
  // 2. Deploy the Web App Server stack once all secrets are defined.
  //---------------------------------------------------------------------------------
  new WebAppServerInfrastructureStack(app, config.stackName, {
    env: {
      account: process.env.CDK_DEFAULT_ACCOUNT,
      region: config.region,
    },
  ...
  //---------------------------------------------------------------------------------
  new WebAppStack(app, config.webApp.stackName, {
    env: {
      account: process.env.CDK_DEFAULT_ACCOUNT,
      region: config.webApp.region,
    },
    config: config,
  });
  app.synth();
